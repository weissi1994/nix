...
printBar()
{
  ...
  local num_bars=$(awk "BEGIN {print int($size * $current / $max)}")
	...
	case $format in
		...
		'0/0')
			if [ -z $(which 'awk' 2>/dev/null) ]; then
				printf "${fc_error} awk not installed${fc_none}"
			else
				#local percent=$('bc' <<< "$value*100/$max")
        local percent=$(awk "BEGIN {print int($value * 100 / $max)}")
...
getNameShell()
{
  local proc_shell=$(readlink /proc/$$/exe)
  printf "%s" "${SHELL:-$proc_shell}"
}
##==============================================================================
##
getDate()
...
getPuppetStatus()
{
  printf "$(getFormattingSequence $(getEffectCode none))$(puppetstatus 2> /dev/null || echo 'Not managed by puppet' )"
}
#!/bin/bash
##  +-----------------------------------+-----------------------------------+
##  |                                                                       |
...
printInfoPuppet()       { printInfoLine "Puppet status" "$(getPuppetStatus)" ; }
...
printInfoJournalctl()
{
	...
	if [ "$journalctl_num_error" -eq "0" ]; then
			local sysctl="No errors since boot"
			local state="nominal"
	elif [ "$journalctl_num_error" -eq "1" ]; then
		local sysctl="1 error since boot!"
		local state="error"
	else
		local sysctl="$journalctl_num_error errors since boot!"
		local state="error"
	fi
...
printInfoCPUTemp()
{
	if ( which sensors > /dev/null 2>&1 && sensors > /dev/null 2>&1); then
		...
		# 	local state="error";
		# fi
    if awk "BEGIN {exit !($current < $high)}"; then
        local state="nominal"
    elif awk "BEGIN {exit !($current < $max)}"; then
        local state="critical"
    else
        local state="error"
    fi
...
printResourceMonitor()
{
	...
	## CHECK STATE
	#local percent=$('bc' <<< "$value*100/$max")
  local percent=$(awk -v value="$value" -v max="$max" 'BEGIN {print int(value * 100 / max)}')
...
printMonitorCPUTemp()
{
    ...
    local crit_percent=$(awk -v high="$high" -v max="$max" 'BEGIN {print int(high * 100 / max)}')
		## PRINT MONITOR
...
reportJournalctl()
{
    ...
    if [ -n "$(pidof systemd)" ]; then
	    journalctl_num_error=$(journalctl --since '3 hours ago' --priority=3 |\
	                           grep -P -o "^\w+\s\d+\s\d{1,2}\:\d{1,2}\:\d{1,2}\s" |\
	                           wc -l)
	    if [ "$journalctl_num_error" -gt "10" ]; then
        printf "\n${fc_crit}JOURNALCTL ERRORS: ${fc_info}(last 10 lines; run 'journalctl --since \"3 hours ago\" --priority=3' for complete info)${fc_none}\n"
			SYSTEMD_COLORS=true journalctl --since '3 hours ago' --priority=3 --no-pager --output=short --no-hostname --no-full -q | tail -n 10
			printf "${fc_none}\n"
	    elif [ "$journalctl_num_error" -ne "0" ] && [ "$journalctl_num_error" -le "10" ]; then
        printf "\n${fc_crit}JOURNALCTL ERRORS: ${fc_info}(run 'journalctl --since \"3 hours ago\" --priority=3' for complete info)${fc_none}\n"
			journalctl --since '3 hours ago' --priority=3 --no-pager --output=short --no-hostname --no-full -q
			printf "${fc_none}\n"
	    fi
    fi
}
...
reportHogsCPU()
{
    ...
    local percent=$(awk -v current="$current" -v max="$max" 'BEGIN {print int(current * 100 / max)}')
...
##==============================================================================
project=$( ( (cat /etc/puppetlabs/puppet/csr_attributes.yaml 2>/dev/null | sed 's/,/\n/g'| grep -oP '1.3.6.1.4.1.34380.1.2.3:\s+\K(.*)') || hostname ) | figlet)
role=$( ( (cat /etc/puppetlabs/puppet/csr_attributes.yaml 2>/dev/null | sed 's/,/\n/g' | grep -oP '1.3.6.1.4.1.34380.1.2.4:\s+\K(.*)') || echo "$USER") | figlet)
location=$( ( (cat /etc/puppetlabs/puppet/csr_attributes.yaml 2>/dev/null | sed 's/,/\n/g' | grep -oP '1.3.6.1.4.1.34380.1.2.5:\s+\K(.*)') || echo "local") | figlet)
logo="\e[32m${location}\e[0m\n\e[35m${project}\e[0m\n\e[34m${role}\e[0m"
##==============================================================================
## STATUS INFO
##
...
local user_config_file="$HOME/.config/nope/synth-shell-greeter.config"
local root_config_file="/etc/nope/os/synth-shell-greeter.root.config"
local sys_config_file="/etc/nope/synth-shell-greeter.config"
...
##==============================================================================
## CHECKS
exitIfCommandsNotAvailable awk
## PRINT TOP SPACER
